...
const UploadForm = ({ onResult }) => {
  ...
  const isFormValid = file && jdText.trim().length >= 3 && jdText.length <= 5000 && Object.keys(validationErrors).length === 0;

  return (
    <div className="max-w-3xl mx-auto glass-card rounded-3xl p-8 md:p-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-10 text-center">
        <h2 className="text-3xl font-bold text-gray-900 mb-2">Resume Analyzer</h2>
        <p className="text-gray-600">Upload your resume and job title or description to get AI-powered insights.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-end mb-1">
            ...
            <span className={`text-xs ${jdText.length > 5000 || jdText.length < 3 ? 'text-amber-600' : 'text-gray-400'}`}>
          ...
          <textarea
            ...
            disabled={isLoading}
            placeholder="Paste the job description or enter a job title (e.g. Java Developer)..."
